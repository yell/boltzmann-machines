...
def main():
    ...
    import matplotlib.pyplot as plt
    from hdm.utils import plot_matrices, gif_matrices

    samples = []
    for i in xrange(32):
        print 100 * i
        V = dbm.sample_v(n_gibbs_steps=100, save_model=True)
        title = 'Samples generated by DBM after {0} Gibbs steps'.format(i)
        _, Z = plot_matrices(V, shape=(28, 28))
        samples.append(Z)

    fig = plt.figure(figsize=(8, 8))
    im, _ = plot_matrices(np.zeros((100, 28, 28)), imshow_params={'cmap': plt.cm.gray, 'animated': True,
                                                                  'vmin': 0., 'vmax': 1.})
    gif_matrices(samples, im, fig, fname,
